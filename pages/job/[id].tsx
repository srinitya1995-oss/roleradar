...
function targetTypeLabel(type: string): string {
  ...
  if (type === "high_signal_connector") return "High-Signal Connector";
  return type;
}
...
export default function JobDetailPage() {
  ...
  useEffect(() => {
    ...
    const refreshTargets = url.searchParams.get("refresh_targets") === "1" ? "?refresh_targets=1" : "";
    fetch(`/api/jobs/${id}${refreshTargets}`)
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error(r.statusText))))
      .then((data: { job: Job; recommendations: Recommendation[]; referral_targets: ReferralTarget[]; eligible_for_connections?: boolean }) => {
        setJob(data.job);
    ...
    <>
      ...
      <div className="inbox-page" style={{ padding: "1.5rem", maxWidth: "56rem", margin: "0 auto" }}>
        <p style={{ marginBottom: "0.75rem" }}>
          <Link href="/" style={{ color: "inherit", textDecoration: "underline" }}>← Inbox</Link>
        </p>
        <div className="job-detail-header">
          ...
          <div className="job-detail-meta">
            ...
            <span className="date-posted">
              Posted {job.date_posted ? formatDate(job.date_posted) : "—"}
            </span>
        ...
        <section className="outreach-section find-connections-section">
          ...
          <p className="find-connections-copy">
            Up to 3 outreach targets: recruiter, hiring manager, high-signal connector. Use the search links to find them on LinkedIn.
            {job.cpi != null && job.cpi < 7 && (
              <span className="find-connections-note"> Best for high-fit roles (CPI ≥ 7).</span>
            )}
          </p>
            {referralTargets.length > 0 && (
            <ul className="outreach-list">
              {referralTargets.map((t) => (
            ...
            {referralTargets.length === 0 && !eligibleForConnections && (
              <p className="outreach-empty">Connections are only auto-generated for Top 5%, Top 20%, or CPI ≥ 7. <a href={`/job/${job.id}?refresh_targets=1`}>Generate targets</a> to run the finder anyway.</p>
            )}
            {referralTargets.length === 0 && eligibleForConnections && (
              <p className="outreach-empty">Loading connections… If this doesn’t update, try <a href={`/job/${job.id}?refresh_targets=1`}>Refresh targets</a> below.</p>
            )}
            <p className="refresh-targets-hint">
              <a href={`/job/${job.id}?refresh_targets=1`}>Refresh targets</a> — re-run connection finder (uses AI when OPENAI_API_KEY is set).
            </p>
          </section>
                ...
                <li key={rec.person.id} className="outreach-card">
                  <div className="outreach-person">
                    ...
                    <div className="outreach-meta">
                      {rec.person.relationship_type && (
                        <span className="rel-type">{rec.person.relationship_type}</span>
                      )}
                      {rec.person.connection_status && (
                        <span className="conn-status">{rec.person.connection_status}</span>
                      )}
                    </div>
                  </div>
                  <div className="outreach-message-preview">
...
.job-detail-header .cpi { font-size: 0.875rem; font-weight: 700; opacity: 1; color: inherit; }
        ...
        .refresh-targets-hint { font-size: 0.8rem; margin-top: 0.75rem; }
        .refresh-targets-hint a { color: #2563eb; text-decoration: underline; }
